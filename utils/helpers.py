...
def make_validation_img(img_, depth_, label, pred):
    ...
    label = np.array([cmap[i.astype(np.uint8) + 1] for i in label])

    pred = np.concatenate(pred)
    pred = np.array([cmap[i.astype(np.uint8) + 1] for i in pred])
    img = img.transpose(1, 2, 0)

    line_0 = np.concatenate([img, depth], 1)
    line_1 = np.concatenate([label, pred], 1)
    return np.concatenate([line_0, line_1], 0)
